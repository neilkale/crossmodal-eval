...
def convert_sample_to_interleaved(sample):
    """
    ...
    """
    text_prompt = sample["final_input_prompt"]
    # This regex finds tokens like <image 1>, <image 2>, etc.
    pattern = re.compile(r'<image\s*(\d+)>')
    
    interleaved = []
    last_idx = 0
    
    # Process each <image N> token found in the prompt
    for match in pattern.finditer(text_prompt):
        start, end = match.span()
        image_num = int(match.group(1))
        image_idx = image_num - 1  # Convert to 0-indexed
        
        # If there is any text before the token, add it as a text element.
        if start > last_idx:
            text_segment = text_prompt[last_idx:start].strip()
            if text_segment:
                interleaved.append({"type": "text", "text": text_segment})
                
        # Add the corresponding image element if available.
        if image_idx < len(sample["image"]):
            interleaved.append({"type": "image", "image": sample["image"][image_idx]})
        else:
            # Optional: handle the case where the image index is out of bounds.
            interleaved.append({"type": "text", "text": f"[Missing image {image_num}]"})
        
        last_idx = end

    # Add any remaining text after the last <image> token.
    if last_idx < len(text_prompt):
        remaining_text = text_prompt[last_idx:].strip()
        if remaining_text:
            interleaved.append({"type": "text", "text": remaining_text})
